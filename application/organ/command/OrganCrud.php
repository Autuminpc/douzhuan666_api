...
class OrganCrud extends Command
{
    protected $stubList = [];
    ...
    protected $fieldSelectpageMap = [
        'nickname' => ['user_id', 'user_ids', 'organ_id', 'organ_ids']
    ];
     ...
     * 保留字段
     */
    protected $reservedField = ['organ_id'];
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $viewDir = $organPath . strtolower(implode(DS, $viewArr)) . DS;

        //最终将生成的文件路径
        $javascriptFile = ROOT_PATH . 'public' . DS . 'assets' . DS . 'js' . DS . 'organend' . DS . $controllerBaseName . '.js';
        $addFile = $viewDir . 'add.html';
        $editFile = $viewDir . 'edit.html';
        $indexFile = $viewDir . 'index.html';
        $recyclebinFile = $viewDir . 'recyclebin.html';
        $langFile = $organPath . 'lang' . DS . Lang::detect() . DS . $controllerBaseName . '.php';
                ...
                if ($v['COLUMN_KEY'] != 'PRI' && !in_array($field, $this->reservedField) && !in_array($field, $this->ignoreFields)) {
                    ...
                    if ($inputType == 'select') {
                        ...
                        if ($this->isMatchSuffix($field, $this->selectpageSuffix)) {
                            ...
                            } elseif ($selectpageController == 'organ') {
                                $attrArr['data-source'] = 'auth/organ/selectpage';
