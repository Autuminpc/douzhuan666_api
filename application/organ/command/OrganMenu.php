...
namespace app\organ\command;

use app\organ\model\AuthRule;
use ReflectionClass;
...
class OrganMenu extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        if (!in_array('all-controller', $controller)) {
            foreach ($controller as $index => $item) {
                ...
                $organPath = dirname(__DIR__) . DS . 'controller' . DS . implode(DS, $controllerArr) . '.php';
                if (!is_file($organPath)) {
            ...
            $controllerDir = $organPath . 'controller' . DS;
            // 扫描新的节点信息并导入
    ...
    protected function importRule($controller)
    {
        ...
        $className = "\\app\\organ\\controller\\" . implode("\\", $controllerArr) . $classSuffix;

        $pathArr = $controllerArr;
        array_unshift($pathArr, '', 'application', 'organ', 'controller');
        ...
        $className = "\\app\\organ\\command\\" . $uniqueName;
        //反射机制调用类的注释和方法名
        ...
        if (isset($matches[1]) && isset($matches[1][0]) && $matches[1][0]) {
            \think\Request::instance()->module('organ');
        ...
        foreach ($controllerArr as $k => $v) {
            ...
            if (!$rulemodel) {
                $this->model
                    ->data(['pid' => $pid, 'name' => $name, 'title' => $title, 'icon' => $icon, 'remark' => $remark, 'ismenu' => 1, 'status' => '1'])
                    ->isUpdate(false)
                    ->save();
                $pid = $this->model->id;
            } else {
        ...
        foreach ($methods as $m => $n) {
            ...
            $ruleArr[] = array('id' => $id, 'pid' => $pid, 'name' => $name . "/" . strtolower($n->name), 'icon' => 'fa fa-circle-o', 'title' => $title, 'ismenu' => 0, 'status' => '1');
        }
