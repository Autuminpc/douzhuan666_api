...
if (!function_exists('build_radios')) {
     ...
     * @return string
     */
    function build_radios($name, $list = [], $selected = null, $extends = null)
    {
        $html = [];
        ...
        return '<div class="radio" '.$extends.'>' . implode(' ', $html) . '</div>';
    }
}
...
if (!function_exists('build_toolbar')) {
    ...
    {
        $auth = \app\organ\library\Auth::instance();
...
if (!function_exists('build_heading')) {
    ...
    function build_heading($path = null, $container = true)
    {
        ...
        $data = Db::name('user_rule')->where('name', $path)->field('title')->find();
        if ($data) {
            $title = __($data['title']);
            $content = __('');
        }
...
if (!function_exists('build_exam_cert')) {
     ...
     * @return string
     */
    function build_exam_cert($exam_time = null, $exam_id)
    {
        return date('YmdHis',$exam_time).str_pad($exam_id,8,'0',STR_PAD_LEFT);
    }
}
