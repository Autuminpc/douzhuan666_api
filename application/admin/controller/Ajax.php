...
class Ajax extends Backend
{
    ...
    public function address_area()
    {
        $params = $this->request->get("row/a");
        if (!empty($params)) {
            $province = isset($params['address_province']) ? $params['address_province'] : '';
            $city = isset($params['address_city']) ? $params['address_city'] : null;
        } else {
            $province = $this->request->get('address_province');
            $city = $this->request->get('address_city');
        }
        $where = ['pid' => 0, 'level' => 1];
        $provincelist = null;
        if ($province !== '') {
