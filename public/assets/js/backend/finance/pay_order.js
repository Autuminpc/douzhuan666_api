define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
	...
	function addFunctionAlty(value, row, index) {
		if(row.status == 0){
		       	return ['<button id="changeStatus" type="button" class="btn btn-default">补单</button>',].join('');
		}
		return '';
	} 
	window.operateEvents = {
	    'click #changeStatus': function (e, value, row, index) {
		    layer.confirm('您确定要将该单据状态更改为支付成功吗？',{btn: ['确定', '取消'],title:"提示"},function(){
			 $.ajax({
			     type: "post",
			     url: 'finance/pay_order/changeStatus',
			     data: row,
			     dataType: "json",
			     async:false,
