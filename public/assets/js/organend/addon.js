define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'template'], function ($, undefined, Backend, Table, Form, Template) {
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: Config.fastorgan.api_url + '/addon/index',
                    add_url: '',
                    edit_url: '',
                    del_url: '',
                    multi_url: ''
            ...
            table.bootstrapTable({
                ...
                queryParams: function (params) {
                    ...
                    $.extend(params, {
                        ...
                        version: Config.fastorgan.version
                    });
                    return params;
                },
                columns: [
                ...
                if (!userinfo) {
                    Layer.open({
                        ...
                        yes: function (index, layero) {
                            Fast.api.ajax({
                                url: Config.fastorgan.api_url + '/user/login',
                                dataType: 'jsonp',
                                data: {
                        ...
                        success: function (layero, index) {
                            $(".layui-layer-btn1", layero).prop("href", "http://www.fastorgan.net/user/register.html").prop("target", "_blank");
                        }
                    });
                } else {
                    Fast.api.ajax({
                        url: Config.fastorgan.api_url + '/user/index',
                        dataType: 'jsonp',
                        data: {
                            user_id: userinfo.id,
                        ...
                        Layer.open({
                            ...
                            yes: function () {
                                Fast.api.ajax({
                                    url: Config.fastorgan.api_url + '/user/logout',
                                    dataType: 'jsonp',
            ...
            var install = function (name, version, force) {
                ...
                Fast.api.ajax({
                    ...
                    data: {
                        ...
                        token: token,
                        version: version,
                        faversion: Config.fastorgan.version
                    }
                }, function (data, ret) {
                    Layer.closeAll();
            ...
            var upgrade = function (name, version) {
                ...
                Fast.api.ajax({
                    url: 'addon/upgrade',
                    data: {name: name, uid: uid, token: token, version: version, faversion: Config.fastorgan.version}
                }, function (data, ret) {
        ...
        api: {
            formatter: {
                ...
                author: function (value, row, index) {
                    ...
                    if (typeof row.homepage !== 'undefined') {
                        ...
                        url = 'https://wpa.qq.com/msgrd?v=3&uin=' + row.qq + '&site=fastorgan.net&menu=yes';
                    }
            ...
            userinfo: {
                get: function () {
                    var userinfo = localStorage.getItem("fastorgan_userinfo");
                    return userinfo ? JSON.parse(userinfo) : null;
                },
                set: function (data) {
                    if (data) {
                        localStorage.setItem("fastorgan_userinfo", JSON.stringify(data));
                    } else {
                        localStorage.removeItem("fastorgan_userinfo");
                    }
                }
            }
        }
