define(['jquery', 'bootstrap', 'backend', 'addtabs', 'organlte', 'form'], function ($, undefined, Backend, undefined, AdminLTE, Form) {
    var Controller = {
        index: function () {
            //双击重新加载页面
            ...
            if (localStorage.getItem("fastep") == "installed") {
                $.ajax({
                    url: Config.fastorgan.api_url + '/addon/recommend',
                    type: 'post',
                    dataType: 'jsonp',
                    success: function (ret) {
                        require(['template'], function (Template) {
                            var install = function (name, title) {
                                Fast.api.ajax({
                                    url: 'addon/install',
                                    data: {name: name, faversion: Config.fastorgan.version}
                                }, function (data, ret) {
            ...
            var checkupdate = function (ignoreversion, tips) {
                $.ajax({
                    url: Config.fastorgan.api_url + '/version/check',
                    type: 'post',
                    data: {version: Config.fastorgan.version},
                    dataType: 'jsonp',
                    success: function (ret) {
            ...
            if (Config.fastorgan.checkupdate && ignoreversion !== "*") {
            ...
            var multiplenav = Config.fastorgan.multiplenav;
            ...
            if ($("ul.sidebar-menu li.active a").size() > 0) {
                ...
                if (Config.fastorgan.multiplenav) {
